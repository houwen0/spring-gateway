...
/**
 * 此过滤器功能：
 * 1.准备：组装请求信息请求body、请求头、请求地址、请求方法等信息
 * 2.请求：代理客户端请求服务端地址
 * 3.返回：将服务端的返回信息返回给客户端（响应头、响应body）
 * @author Spencer Gibb
...
public class NettyRoutingFilter implements GlobalFilter, Ordered {
	...
	public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
		...
		//获取请求地址协议，如果不是http或者https，或者重复进入的，则直接返回
		...
		/**
		 * 构建httpClient，代理客户端去访问服务器端，需要将客户端的请求信息放入到httpClient中，具体包括：
		 * 1.请求头信息
		 * 2.请求方法
		 * 3.请求地址
		 * 4.请求body信息
		 * 5.最后将服务器端获取到的信息进行加工返回给客户端response中，且网关关于客户端的信息，都可以从exchange中获取和放入
		 */

		Flux<HttpClientResponse> responseFlux = getHttpClient(route)
				.headers(headers -> {
					...
					//调用netty的writeAndFlush，将body内容发送到请求地址
					...
					if (!filteredResponseHeaders
						...
						//transfer-encoding与content-length不能同时存在，transfer-encoding是分块编码，而content-length是按照长度进行解析的,以transfer-encoding为主
	 ...
	 * @return
	 */
	protected HttpClient getHttpClient(Route route) {
